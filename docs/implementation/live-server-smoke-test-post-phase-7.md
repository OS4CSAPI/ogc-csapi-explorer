...
| **P5-F2** — Label-only properties dropped by normalizer      | Unchanged    | **RESOLVED** _(post-report)_ | `normalizeObservedProperties()` now falls back to `label` when `definition` absent ([#165](https://github.com/OS4CSAPI/ogc-client-CSAPI_2/issues/165)) |
...
| `parseValidTime`      | End sentinel           | `"now"` (Part 2)                                                                         | `".."` (Part 1 GeoJSON)                | ✅ _(post-report fix)_ `".."` now recognized alongside `"now"` ([#162](https://github.com/OS4CSAPI/ogc-client-CSAPI_2/issues/162))                                                                      |
...
| P5-F2   | Unchanged | **RESOLVED** _(post-report)_ | `normalizeObservedProperties()` label fallback ([#165](https://github.com/OS4CSAPI/ogc-client-CSAPI_2/issues/165)) |
...
~~The library is ready for clean-pr merge pending resolution of deferred findings P7-F2 and P7-F3 (recommended but not mandatory).~~ _(P7-F3 was retracted as a testing artifact — see above.)_
